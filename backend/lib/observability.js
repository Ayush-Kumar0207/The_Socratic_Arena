...
const METRICS_CACHE_MS = Number(process.env.METRICS_CACHE_MS || 1000);
let cachedMetrics = null;
let cachedMetricsUntil = 0;
...
const cognitiveInsightsTotal = new client.Counter({
  name: `${METRIC_PREFIX}cognitive_insights_total`,
  help: 'Realtime cognitive debate insights generated by the backend.',
  labelNames: ['severity', 'speaker'],
});

const cognitiveRiskScore = new client.Histogram({
  name: `${METRIC_PREFIX}cognitive_risk_score`,
  help: 'Realtime cognitive risk score for debate turns.',
  labelNames: ['severity', 'speaker'],
  buckets: [0.05, 0.15, 0.3, 0.5, 0.7, 0.9, 1],
});
...
export const metricsHandler = async (req, res, next) => {
  try {
    const now = Date.now();
    if (!cachedMetrics || now >= cachedMetricsUntil) {
      cachedMetrics = await register.metrics();
      cachedMetricsUntil = now + METRICS_CACHE_MS;
    }
...
export const recordCognitiveInsight = ({ severity, speaker, riskScore }) => {
  const labels = {
    severity: sanitizeLabel(severity),
    speaker: sanitizeLabel(speaker),
  };
  cognitiveInsightsTotal.inc(labels);
  cognitiveRiskScore.observe(labels, Math.max(0, Math.min(1, riskScore || 0)));
};
